...
namespace Big
{
	...
	public:
		...
		void EndCall(std::uint64_t hash)
		{
			...
			{
				if (void *handler = GetHandler(*newHash))
				{
					// reinterpret_cast<void (*)(rage::scrNativeCallContext *)>(handler);
					// ((void (*)(rage::scrNativeCallContext *))(handler))(&m_Context);
					g_Logger->Info("Invoking native %p", hash);
					reinterpret_cast<void (*)(rage::scrNativeCallContext *)>(handler)(&this->m_Context);
					// static_cast<void (*)(rage::scrNativeCallContext *)>(handler)(&this->m_Context);
				}
				else
				{
