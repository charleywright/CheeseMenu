...
namespace Cheese
{
	...
	{
		MemoryHandle gameBuildSig = Signature("48 83 EC 60 48 8D 0D ? ? ? ? E8").Scan().Sub(17);
		m_GameBuild = gameBuildSig.Add(265 + 3).Rip().As<decltype(m_GameBuild)>();

		MemoryHandle worldPtrSig = Signature("48 8B 05 ? ? ? ? 45 ? ? ? ? 48 8B 48 08 48 85 C9 74 07").Scan();
		m_WorldPtr = worldPtrSig.Add(*worldPtrSig.Add(3).As<int *>()).Add(7).As<decltype(m_WorldPtr)>();
	}
